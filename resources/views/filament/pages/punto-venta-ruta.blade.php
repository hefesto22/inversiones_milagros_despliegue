        ...
        <div class="space-y-6">
            ...
            <div class="p-3 bg-green-50 dark:bg-green-900/30 rounded-lg border border-green-200 dark:border-green-700">
                <div class="flex items-center gap-2 text-green-800 dark:text-green-200">
                    ...
                    <span class="text-sm text-green-600 dark:text-green-300">({{ ucfirst($clienteSeleccionado->tipo) }})</span>
                    @endif
                </div>
            </div>
            @endif
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-800">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Unidad</th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Precio Base</th>
                            <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">ISV</th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Precio Cliente</th>
                    ...
                    <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        @endphp
                        <tr class="hover:bg-gray-100/50 dark:hover:bg-gray-700/50 transition {{ $bajoMinimo ? 'bg-red-50 dark:bg-red-900/20' : '' }}">
                            ...
                            <td class="px-4 py-3 text-center">
                                <span class="inline-flex px-2.5 py-1 text-xs font-medium rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300">
                                    {{ $item['unidad'] }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3 text-center">
                                @if ($item['aplica_isv'])
                                <span class="inline-flex px-2.5 py-1 text-xs font-medium rounded-full bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300">
                                    +L {{ number_format($item['monto_isv'], 2) }}
                                </span>
                                @else
                                <span class="inline-flex px-2.5 py-1 text-xs font-medium rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">
                                    —
                                </span>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-2">
                                    <button wire:click="modificarCantidad('{{ $item['uid'] }}', -1)"
                                        class="w-8 h-8 flex items-center justify-center rounded-full bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-gray-700 dark:text-gray-200 transition">
                            ...
                            <td class="px-4 py-3 text-right text-sm font-bold text-primary-600 dark:text-primary-400">
                                L {{ number_format($subtotal, 2) }}
                            </td>
                            <td class="px-4 py-3 text-center">
                                <button wire:click="quitarDelCarrito('{{ $item['uid'] }}')"
                                    class="p-1.5 rounded-full text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-100 dark:hover:bg-red-900/30 transition">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800 p-4 space-y-2 text-sm">
                        <div class="flex justify-between text-gray-600 dark:text-gray-300">
                            <span>Subtotal (sin ISV)</span>
                            <span class="font-medium text-gray-900 dark:text-white">L {{ number_format($this->subtotalSinISV, 2) }}</span>
                        </div>
                        <div class="flex justify-between text-gray-600 dark:text-gray-300">
                            <span>ISV (15%)</span>
                            <span class="font-medium text-gray-900 dark:text-white">L {{ number_format($this->totalISV, 2) }}</span>
                        </div>
                        <div class="flex justify-between text-gray-600 dark:text-gray-300">
                            <span>Subtotal Bruto</span>
                            <span class="font-medium text-gray-900 dark:text-white">L {{ number_format($this->subtotalBruto, 2) }}</span>
                        </div>
                        <div class="pt-3 border-t border-gray-200 dark:border-gray-600">
                            <label class="block text-gray-700 dark:text-gray-300 mb-1.5 font-medium">Descuento</label>
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 font-medium">L</span>
                                <input type="number" wire:model.live="descuento" step="0.01" min="0"
                                    class="w-full pl-8 pr-3 py-2.5 rounded-lg border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-primary-500 focus:border-primary-500" />
                            </div>
                        </div>
                        <div class="pt-4 border-t border-gray-200 dark:border-gray-600 flex justify-between items-center">
                ...
                <div class="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800 p-4 space-y-4">
                        <div>
                            <label class="block text-sm text-gray-700 dark:text-gray-300 mb-1.5 font-medium">Tipo de Pago</label>
                            <select wire:model="tipo_pago"
                                class="w-full px-3 py-2.5 rounded-lg border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:ring-primary-500 focus:border-primary-500">
                                <option value="contado">Contado</option>
                                <option value="credito">Crédito</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm text-gray-700 dark:text-gray-300 mb-1.5 font-medium">Observaciones</label>
                            <textarea wire:model="observaciones" rows="3" placeholder="Notas adicionales..."
                                class="w-full px-3 py-2.5 rounded-lg border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 text-sm focus:ring-primary-500 focus:border-primary-500"></textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 max-w-md w-full mx-4 border border-gray-200 dark:border-gray-700">
                    <div class="text-center">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">
                            Cliente: <strong class="text-gray-700 dark:text-gray-200">{{ $clienteSeleccionado?->nombre ?? '---' }}</strong><br>
                            Tipo de pago: <strong class="text-gray-700 dark:text-gray-200">{{ ucfirst($this->tipo_pago) }}</strong>
                        </p>
                        <div class="flex gap-3 justify-center">
                            <button wire:click="cancelarConfirmacion"
                                class="px-6 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-700 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition">
                                Cancelar
                            </button>
                            <button wire:click="procesarVenta"
                                wire:loading.attr="disabled"
                                style="background-color: #22c55e !important;"
                                class="px-6 py-2.5 text-sm font-semibold text-white rounded-lg hover:opacity-90 transition disabled:opacity-50">
